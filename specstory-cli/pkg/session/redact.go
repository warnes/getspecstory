...
// context rules put the secret in group 1).
...
var builtinPatterns = []redactPattern{
	...
	{re: regexp.MustCompile(`\bgghp_[A-Za-z0-9]{20,}`), label: "GITHUB_MODELS_TOKEN"},
	// No gitleaks rule covers Groq keys (v8.30.1).
	{re: regexp.MustCompile(`\bgsk_[A-Za-z0-9]{20,}`), label: "GROQ_API_KEY"},
	...
	{re: regexp.MustCompile(`\bsk-ant-[A-Za-z0-9_-]{20,}`), label: "ANTHROPIC_API_KEY"},
	...
	{re: regexp.MustCompile(`\bsk-proj-[A-Za-z0-9_-]{20,}`), label: "OPENAI_PROJECT_KEY"},
}
...
var fallbackPatterns = []redactPattern{
	...
	{re: regexp.MustCompile(`\bsk-[A-Za-z0-9]{40,}`), label: "OPENAI_API_KEY"},
	...
	{re: regexp.MustCompile(`(?i)aws[_\-\s.]{0,4}secret[_\-\s.]{0,4}(?:access[_\-\s.]{0,4})?key[_\-\s.]{0,10}[=:]\s*["']?([A-Za-z0-9/+=]{40})\b`), label: "AWS_SECRET_ACCESS_KEY", group: 1},
	...
	{re: regexp.MustCompile(`\b[a-zA-Z][a-zA-Z0-9+.-]*://[^:/\s@'"]+:([^@\s'"]{3,})@`), label: "URL_PASSWORD", group: 1},
}
...
func RedactContent(content string, extraPatterns []string) string {
	...
	for _, p := range compileExtras(extraPatterns) {
		content = p.apply(content)
	}
	return content
}
