...
- `specstory monitor` now also detects VS Code Copilot IDE chat activity (stock VS Code, Insiders, VSCodium, and VSCodium Insiders) by selectively watching only the workspaceStorage entries whose workspace maps into the discovered repos; Cursor IDE activity detection is deferred because Cursor stores chat activity in SQLite databases (global and per-workspace `state.vscdb`), which requires polling machinery rather than path mapping.
